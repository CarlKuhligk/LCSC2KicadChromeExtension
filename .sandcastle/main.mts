...
// We install both dep sets so Pytest and Vitest can run in the sandbox.
const hooks = {
  sandbox: {
    onSandboxReady: [
      { command: "pip install --user --break-system-packages -r requirements.txt -r requirements-dev.txt" },
      { command: "cd chrome_extension && npm install" },
    ],
  },
};

// Copy chrome_extension/node_modules from host to speed up Vitest startup.
// The npm install hook handles platform-specific binaries and any packages
// added since the last copy.
const copyToWorktree = ["chrome_extension/node_modules"];
