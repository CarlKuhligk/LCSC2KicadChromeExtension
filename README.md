...
- **Packaged server (PyInstaller)** — CI and release builds embed the same graphic as the extension: **`chrome_extension/icon.png`**. Windows builds use a generated **`app.ico`** (`packaging/pyinstaller/build_windows_ico.py`, needs **Pillow**); macOS uses **`app.icns`** from **`packaging/pyinstaller/build_macos_icns.sh`** (`sips` + `iconutil`). Linux onefile binaries do not get a custom executable icon from PyInstaller. Example (Windows): `pip install pyinstaller pillow` → `python packaging/pyinstaller/build_windows_ico.py` → `pyinstaller --onefile --icon build/pyinstaller/app.ico --name "KiCad Parts Importer" run_server.py`.
